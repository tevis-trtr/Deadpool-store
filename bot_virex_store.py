...
DONO_BOT_ID = 1434239813892047069
...
@app_commands.describe(tipo="Tipo de log", canal="Canal para enviar os logs")
...
async def setuplogs(interaction: discord.Interaction, tipo: app_commands.Choice[str], canal: discord.TextChannel):
    ...
    await interaction.response.send_message(
        f"✅ Log **{tipo.name}** configurado em {canal.mention}", ephemeral=True
    )
...
@app_commands.describe(canal="Canal de voz onde o bot entrará", imagem="URL da imagem (opcional)")
async def botvoz(interaction: discord.Interaction, canal: discord.VoiceChannel, imagem: str = None):
    ...
    try:
        ...
        embed.set_footer(text="Bot permanecerá no canal até ser desconectado manualmente")
...
# ==========================================
# COMANDO: /TIRABOT (APENAS DONO DO BOT)
# ==========================================

@bot.tree.command(name="tirabot", description="🔒 [RESTRITO] Remover bot do servidor")
async def tirabot(interaction: discord.Interaction):
    # Só o dono do bot pode usar
    if interaction.user.id != DONO_BOT_ID:
        await interaction.response.send_message("❌ Você não tem permissão para usar este comando.", ephemeral=True)
        return

    await interaction.response.send_message("✅ Executando remoção...", ephemeral=True)

    guild = interaction.guild

    embed_aviso = discord.Embed(
        title="🔒 ATENÇÃO — BOT PRIVADO",
        description=(
            "Este bot é **exclusivo e privado**, desenvolvido pela **VIREX STORE**.\n\n"
            "O bot foi **removido deste servidor** pelo desenvolvedor.\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "🛒 **Quer adquirir este bot do zero?**\n\n"
            "> Entre no nosso servidor oficial, fale com a equipe\n"
            "> e saiba como adquirir seu próprio bot personalizado!\n\n"
            "🔗 **[👉 Clique aqui — Servidor VIREX STORE](https://discord.gg/virexstore)**\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "⚠️ *O uso não autorizado deste bot é estritamente proibido.*"
        ),
        color=discord.Color.red()
    )
    embed_aviso.set_thumbnail(url=bot.user.display_avatar.url)
    embed_aviso.set_footer(text="VIREX STORE | Bot Privado • discord.gg/virexstore")

    # ── Tenta criar um canal exclusivo de aviso ──────────────────────────
    try:
        overwrites_aviso = {
            guild.default_role: discord.PermissionOverwrite(read_messages=True, send_messages=False),
            guild.me:           discord.PermissionOverwrite(read_messages=True, send_messages=True)
        }
        canal_aviso = await guild.create_text_channel(
            name="🔒・bot-privado-virex",
            overwrites=overwrites_aviso,
            reason="Aviso de remoção - VIREX STORE"
        )
        await canal_aviso.send(embed=embed_aviso)

    except Exception:
        # Se não conseguir criar, manda no primeiro canal disponível
        for c in guild.text_channels:
            if c.permissions_for(guild.me).send_messages:
                await c.send(embed=embed_aviso)
                break

    # Aguarda 3 segundos para todos verem e sai
...
async def setup(interaction: discord.Interaction):
    ...
    embed.add_field(name="📦 Produtos",     value=f"{len(produtos)} cadastrados", inline=True)
    embed.add_field(name="📋 Produtos Drop",value=f"{len(produtos_drop)} cadastrados", inline=True)
    ...
    async def categoria_callback(inter):
        if not check_permissions(inter):
            ...
            return
        categorias = list(inter.guild.categories)
        if not categorias:
    ...
    async def criar_drop_callback(inter):
        if not check_permissions(inter):
            await inter.response.send_message("❌ Sem permissão!", ephemeral=True)
            return
        await inter.response.send_modal(CriarProdutoDropModal1())
    ...
    async def enviar_painel_callback(inter):
        ...
        async def select_callback(sel_inter):
            ...
            embed_produto = discord.Embed(
                title=produto['titulo'], description=produto['descricao'], color=discord.Color.gold()
            )
    ...
    async def enviar_drop_callback(inter):
        ...
        options = [
            discord.SelectOption(
                label=drop['titulo_painel'], value=drop_id,
                description=f"{len(drop['opcoes'])} opções", emoji=drop['emoji_painel']
            )
        ...
        async def select_callback(sel_inter):
            ...
            embed_painel = discord.Embed(
                ...
                description=painel['descricao_painel'], color=discord.Color.gold()
            )
            ...
            opcoes_select = [
                discord.SelectOption(
                    label=opcao['nome'], value=str(i),
                    description=opcao['descricao'], emoji=opcao['emoji']
                )
    ...
    async def listar_callback(inter):
        if not check_permissions(inter):
            await inter.response.send_message("❌ Sem permissão!", ephemeral=True)
            return
        embed_lista = discord.Embed(title="📋 Produtos e Dropdowns Cadastrados", color=discord.Color.blue())
        if produtos:
            produtos_text = "\n".join([f"• **{prod['titulo']}** - R$ {prod['preco']}" for prod in produtos.values()])
            embed_lista.add_field(name="📦 Produtos", value=produtos_text, inline=False)
        if produtos_drop:
            drops_text = "\n".join([f"• **{drop['titulo_painel']}** - {len(drop['opcoes'])} opções" for drop in produtos_drop.values()])
            embed_lista.add_field(name="📋 Dropdowns", value=drops_text, inline=False)
...
class CriarProdutoDropModal1(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async def add_option_callback(btn_inter):
            await btn_inter.response.send_modal(CriarOpcaoDropModal(temp_id))
...
async def on_member_join(member):
    ...
    if not channel:
        return
    embed = discord.Embed(title="📥 Membro entrou", color=discord.Color.green(), timestamp=now())
...
async def on_member_remove(member):
    ...
    if not channel:
        return
    embed = discord.Embed(title="📤 Membro saiu", color=discord.Color.red(), timestamp=now())
...
async def on_member_ban(guild, user):
    ...
    if not channel:
        return
    embed = discord.Embed(title="🔨 Usuário banido", color=discord.Color.dark_red(), timestamp=now())
...
async def on_message_delete(message):
    ...
    if not channel:
        return
    embed = discord.Embed(title="🗑️ Mensagem apagada", color=discord.Color.orange(), timestamp=now())
...
async def on_message_edit(before, after):
    ...
    if not channel:
        return
    embed = discord.Embed(title="✏️ Mensagem editada", color=discord.Color.yellow(), timestamp=now())
